#include "TouRangedWeaponInstance.h"
#include "NativeGameplayTags.h"
#include "GameFramework/Pawn.h"
#include "GameFramework/CharacterMovementComponent.h"
#include "Camera/TouCameraComponent.h"
#include "Physics/PhysicalMaterialWithTags.h"
#include "Weapons/TouWeaponInstance.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(TouRangedWeaponInstance)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Tou_Weapon_SteadyAimingCamera, "Tou.Weapon.SteadyAimingCamera");

UTouRangedWeaponInstance::UTouRangedWeaponInstance(const FObjectInitializer& ObjectInitializer)
...
void UTouRangedWeaponInstance::PostLoad()
{
	Super::PostLoad();
...
#if WITH_EDITOR
void UTouRangedWeaponInstance::PostEditChangeProperty(struct FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);
	UpdateDebugVisualization();
}

void UTouRangedWeaponInstance::UpdateDebugVisualization()
{
...
void UTouRangedWeaponInstance::OnEquipped()
{
	Super::OnEquipped();
...
void UTouRangedWeaponInstance::OnUnequipped()
{
	Super::OnUnequipped();
}

void UTouRangedWeaponInstance::Tick(float DeltaSeconds)
{
	APawn* Pawn = GetPawn();
	check(Pawn != nullptr);
...
void UTouRangedWeaponInstance::ComputeHeatRange(float& MinHeat, float& MaxHeat)
{
	float Min1;
	float Max1;
...
void UTouRangedWeaponInstance::ComputeSpreadRange(float& MinSpread, float& MaxSpread)
{
	HeatToSpreadCurve.GetRichCurveConst()->GetValueRange(/*out*/ MinSpread, /*out*/ MaxSpread);
}

void UTouRangedWeaponInstance::AddSpread()
{
...
float UTouRangedWeaponInstance::GetDistanceAttenuation(float Distance, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
	const FRichCurve* Curve = DistanceDamageFalloff.GetRichCurveConst();
	return Curve->HasAnyData() ? Curve->Eval(Distance) : 1.0f;
}

float UTouRangedWeaponInstance::GetPhysicalMaterialAttenuation(const UPhysicalMaterial* PhysicalMaterial, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags) const
{
...
bool UTouRangedWeaponInstance::UpdateSpread(float DeltaSeconds)
{
...
bool UTouRangedWeaponInstance::UpdateMultipliers(float DeltaSeconds)
{
	...
	if (const UTouCameraComponent* CameraComponent = UTouCameraComponent::FindCameraComponent(Pawn))
	{
		...
		AimingAlpha = (TopCameraTag == TAG_Tou_Weapon_SteadyAimingCamera) ? TopCameraWeight : 0.0f;
	}
