#include "TouGameplayAbility_RangedWeapon.h"
#include "Weapons/TouRangedWeaponInstance.h"
#include "Physics/TouCollisionChannels.h"
#include "TouLogChannels.h"
#include "AIController.h"
#include "NativeGameplayTags.h"
#include "Weapons/TouWeaponStateComponent.h"
#include "AbilitySystemComponent.h"
#include "AbilitySystem/TouGameplayAbilityTargetData_SingleTargetHit.h"
#include "DrawDebugHelpers.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(TouGameplayAbility_RangedWeapon)
...
namespace TouConsoleVariables
{
	...
	static FAutoConsoleVariableRef CVarDrawBulletTraceDuraton(
		TEXT("tou.Weapon.DrawBulletTraceDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHits(
		TEXT("tou.Weapon.DrawBulletHitDuration"),
	...
	static FAutoConsoleVariableRef CVarDrawBulletHitRadius(
		TEXT("tou.Weapon.DrawBulletHitRadius"),
		DrawBulletHitRadius,
...
UTouGameplayAbility_RangedWeapon::UTouGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
	SourceBlockedTags.AddTag(TAG_WeaponFireBlocked);
}

UTouRangedWeaponInstance* UTouGameplayAbility_RangedWeapon::GetWeaponInstance() const
{
	return Cast<UTouRangedWeaponInstance>(GetAssociatedEquipment());
}

bool UTouGameplayAbility_RangedWeapon::CanActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayTagContainer* SourceTags, const FGameplayTagContainer* TargetTags, FGameplayTagContainer* OptionalRelevantTags) const
{
	...
	if (bResult)
	{
		...
		{
			UE_LOG(LogTouAbilitySystem, Error, TEXT("Weapon ability %s cannot be activated because there is no associated ranged weapon (equipment instance=%s but needs to be derived from %s)"),
				*GetPathName(),
				*GetPathNameSafe(GetAssociatedEquipment()),
				*UTouRangedWeaponInstance::StaticClass()->GetName());
			bResult = false;
		}
	}
...
int32 UTouGameplayAbility_RangedWeapon::FindFirstPawnHitResult(const TArray<FHitResult>& HitResults)
{
...
void UTouGameplayAbility_RangedWeapon::AddAdditionalTraceIgnoreActors(FCollisionQueryParams& TraceParams) const
{
...
ECollisionChannel UTouGameplayAbility_RangedWeapon::DetermineTraceChannel(FCollisionQueryParams& TraceParams, bool bIsSimulated) const
{
	return Tou_TraceChannel_Weapon;
}

FHitResult UTouGameplayAbility_RangedWeapon::WeaponTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHitResults) const
{
...
FVector UTouGameplayAbility_RangedWeapon::GetWeaponTargetingSourceLocation() const
{
...
FTransform UTouGameplayAbility_RangedWeapon::GetTargetingTransform(APawn* SourcePawn, ETouAbilityTargetingSource Source) const
{
	check(SourcePawn);
	AController* SourcePawnController = SourcePawn->GetController(); 
	UTouWeaponStateComponent* WeaponStateComponent = (SourcePawnController != nullptr) ? SourcePawnController->FindComponentByClass<UTouWeaponStateComponent>() : nullptr;

	// The caller should determine the transform without calling this if the mode is custom!
	check(Source != ETouAbilityTargetingSource::Custom);
	...
	if ((Controller != nullptr) && ((Source == ETouAbilityTargetingSource::CameraTowardsFocus) || (Source == ETouAbilityTargetingSource::PawnTowardsFocus) || (Source == ETouAbilityTargetingSource::WeaponTowardsFocus)))
	{
		...
		if (Source == ETouAbilityTargetingSource::CameraTowardsFocus)
		{
	...
	if ((Source == ETouAbilityTargetingSource::WeaponForward) || (Source == ETouAbilityTargetingSource::WeaponTowardsFocus))
	{
		SourceLoc = GetWeaponTargetingSourceLocation();
	}
	else
	...
	if (bFoundFocus && ((Source == ETouAbilityTargetingSource::PawnTowardsFocus) || (Source == ETouAbilityTargetingSource::WeaponTowardsFocus)))
	{
...
FHitResult UTouGameplayAbility_RangedWeapon::DoSingleBulletTrace(const FVector& StartTrace, const FVector& EndTrace, float SweepRadius, bool bIsSimulated, OUT TArray<FHitResult>& OutHits) const
{
#if ENABLE_DRAW_DEBUG
	if (TouConsoleVariables::DrawBulletTracesDuration > 0.0f)
	{
		static float DebugThickness = 1.0f;
		DrawDebugLine(GetWorld(), StartTrace, EndTrace, FColor::Red, false, TouConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
	}
...
void UTouGameplayAbility_RangedWeapon::PerformLocalTargeting(OUT TArray<FHitResult>& OutHits)
{
	APawn* const AvatarPawn = Cast<APawn>(GetAvatarActorFromActorInfo());
	...
	if (AvatarPawn && AvatarPawn->IsLocallyControlled() && WeaponData)
	{
		...
		const FTransform TargetTransform = GetTargetingTransform(AvatarPawn, ETouAbilityTargetingSource::CameraTowardsFocus);
		...
		if (TouConsoleVariables::DrawBulletTracesDuration > 0.0f)
		{
			...
			DrawDebugLine(GetWorld(), InputData.StartTrace, InputData.StartTrace + (InputData.AimDir * 100.0f), FColor::Yellow, false, TouConsoleVariables::DrawBulletTracesDuration, 0, DebugThickness);
		}
#endif
...
void UTouGameplayAbility_RangedWeapon::TraceBulletsInCartridge(const FRangedWeaponFiringInput& InputData, OUT TArray<FHitResult>& OutHits)
{
	UTouRangedWeaponInstance* WeaponData = InputData.WeaponData;
	check(WeaponData);
	...
	for (int32 BulletIndex = 0; BulletIndex < BulletsPerCartridge; ++BulletIndex)
	{
		...
		{
#if ENABLE_DRAW_DEBUG
			if (TouConsoleVariables::DrawBulletHitDuration > 0.0f)
			{
				DrawDebugPoint(GetWorld(), Impact.ImpactPoint, TouConsoleVariables::DrawBulletHitRadius, FColor::Red, false, TouConsoleVariables::DrawBulletHitRadius);
			}
#endif
...
void UTouGameplayAbility_RangedWeapon::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
...
void UTouGameplayAbility_RangedWeapon::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled)
{
...
void UTouGameplayAbility_RangedWeapon::OnTargetDataReadyCallback(const FGameplayAbilityTargetDataHandle& InData, FGameplayTag ApplicationTag)
{
	UAbilitySystemComponent* MyAbilityComponent = CurrentActorInfo->AbilitySystemComponent.Get();
	check(MyAbilityComponent);
	...
	if (const FGameplayAbilitySpec* AbilitySpec = MyAbilityComponent->FindAbilitySpecFromHandle(CurrentSpecHandle))
	{
		...
		if (!bProjectileWeapon)
		{
			if (AController* Controller = GetControllerFromActorInfo())
			{
				...
				{
					// Confirm hit markers
					if (UTouWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UTouWeaponStateComponent>())
					{
		...
		{
			UE_LOG(LogTouAbilitySystem, Warning, TEXT("Weapon ability %s failed to commit (bIsTargetDataValid=%d)"), *GetPathName(), bIsTargetDataValid ? 1 : 0);
			K2_EndAbility();
		}
	}
...
void UTouGameplayAbility_RangedWeapon::StartRangedWeaponTargeting()
{
	...
	check(Controller);
	UTouWeaponStateComponent* WeaponStateComponent = Controller->FindComponentByClass<UTouWeaponStateComponent>();
	...
	if (FoundHits.Num() > 0)
	{
		...
		{
			FTouGameplayAbilityTargetData_SingleTargetHit* NewTargetData = new FTouGameplayAbilityTargetData_SingleTargetHit();
