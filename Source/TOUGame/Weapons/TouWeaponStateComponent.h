...
#include "TouWeaponStateComponent.generated.h"
...
struct FTouScreenSpaceHitLocation
{
...
struct FTouServerSideHitMarkerBatch
{
	FTouServerSideHitMarkerBatch() { }

	FTouServerSideHitMarkerBatch(uint8 InUniqueId) :
		UniqueId(InUniqueId)
	{ }

	TArray<FTouScreenSpaceHitLocation> Markers;
...
UCLASS()
class UTouWeaponStateComponent : public UControllerComponent
{
	GENERATED_BODY()

public:

	UTouWeaponStateComponent(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	void GetLastWeaponDamageScreenLocations(TArray<FTouScreenSpaceHitLocation>& WeaponDamageScreenLocations)
	{
...
private:
	...
	TArray<FTouScreenSpaceHitLocation> LastWeaponDamageScreenLocations;

	/** The unconfirmed hits */
	TArray<FTouServerSideHitMarkerBatch> UnconfirmedServerSideHitMarkers;
};
