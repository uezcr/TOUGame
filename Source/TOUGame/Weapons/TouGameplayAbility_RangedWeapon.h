...
#include "Equipment/TouGameplayAbility_FromEquipment.h"

#include "TouGameplayAbility_RangedWeapon.generated.h"

enum ECollisionChannel : int;

class APawn;
class UTouRangedWeaponInstance;
class UObject;
...
UENUM(BlueprintType)
enum class ETouAbilityTargetingSource : uint8
{
...
/**
 * UTouGameplayAbility_RangedWeapon
 *
 * An ability granted by and associated with a ranged weapon instance
 */
UCLASS()
class UTouGameplayAbility_RangedWeapon : public UTouGameplayAbility_FromEquipment
{
	GENERATED_BODY()

public:

	UTouGameplayAbility_RangedWeapon(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());

	UFUNCTION(BlueprintCallable, Category="Tou|Ability")
	UTouRangedWeaponInstance* GetWeaponInstance() const;
...
protected:
	struct FRangedWeaponFiringInput
	{
		...
		UTouRangedWeaponInstance* WeaponData = nullptr;
...
protected:
	...
	FTransform GetTargetingTransform(APawn* SourcePawn, ETouAbilityTargetingSource Source) const;
