...
#include "TouSettingsShared.generated.h"
...
UENUM(BlueprintType)
enum class ETouAllowBackgroundAudioSetting : uint8
{
	Off,
	AllSounds,
...
/**
 * UTouSettingsShared - The "Shared" settings are stored as part of the USaveGame system, these settings are not machine
 ...
 */
UCLASS()
class UTouSettingsShared : public ULocalPlayerSaveGame
{
	GENERATED_BODY()

public:
	DECLARE_EVENT_OneParam(UTouSettingsShared, FOnSettingChangedEvent, UTouSettingsShared* Settings);
...
public:
	...
	static UTouSettingsShared* CreateTemporarySettings(const UTouLocalPlayer* LocalPlayer);
	
	/** Synchronously loads a settings object, this is not valid to call before login */
	static UTouSettingsShared* LoadOrCreateSettings(const UTouLocalPlayer* LocalPlayer);

	DECLARE_DELEGATE_OneParam(FOnSettingsLoadedEvent, UTouSettingsShared* Settings);

	/** Starts an async load of the settings object, calls Delegate on completion */
	static bool AsyncLoadOrCreateSettings(const UTouLocalPlayer* LocalPlayer, FOnSettingsLoadedEvent Delegate);
...
public:
	UFUNCTION()
	ETouAllowBackgroundAudioSetting GetAllowAudioInBackgroundSetting() const { return AllowAudioInBackground; }
	UFUNCTION()
	void SetAllowAudioInBackgroundSetting(ETouAllowBackgroundAudioSetting NewValue);

	void ApplyBackgroundAudioSettings();

private:
	UPROPERTY()
	ETouAllowBackgroundAudioSetting AllowAudioInBackground = ETouAllowBackgroundAudioSetting::Off;
...
public:
	UFUNCTION()
	ETouGamepadSensitivity GetGamepadLookSensitivityPreset() const { return GamepadLookSensitivityPreset; }
	UFUNCTION()
	void SetLookSensitivityPreset(ETouGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadLookSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	UFUNCTION()
	ETouGamepadSensitivity GetGamepadTargetingSensitivityPreset() const { return GamepadTargetingSensitivityPreset; }
	UFUNCTION()
	void SetGamepadTargetingSensitivityPreset(ETouGamepadSensitivity NewValue) { ChangeValueAndDirty(GamepadTargetingSensitivityPreset, NewValue); ApplyInputSensitivity(); }

	void ApplyInputSensitivity();
	
private:
	UPROPERTY()
	ETouGamepadSensitivity GamepadLookSensitivityPreset = ETouGamepadSensitivity::Normal;
	UPROPERTY()
	ETouGamepadSensitivity GamepadTargetingSensitivityPreset = ETouGamepadSensitivity::Normal;
