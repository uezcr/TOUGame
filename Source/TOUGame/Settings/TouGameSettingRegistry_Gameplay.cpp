...
#include "CustomSettings/TouSettingValueDiscrete_Language.h"
...
UGameSettingCollection* UTouGameSettingRegistry::InitializeGameplaySettings(UTouLocalPlayer* InLocalPlayer)
{
...

	{
		...
		//----------------------------------------------------------------------------------
		{
			UTouSettingValueDiscrete_Language* Setting = NewObject<UTouSettingValueDiscrete_Language>();
...

	{
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UTouSettingsLocal>()->ShouldAutoRecordReplays());

			Setting->AddEditCondition(FWhenPlayingAsPrimaryPlayer::Get());
			Setting->AddEditCondition(FWhenPlatformHasTrait::KillIfMissing(UTouReplaySubsystem::GetPlatformSupportTraitTag(), TEXT("Platform does not support saving replays")));
		...
		//----------------------------------------------------------------------------------
		{
			...
			Setting->SetDefaultValue(GetDefault<UTouSettingsLocal>()->GetNumberOfReplaysToKeep());
