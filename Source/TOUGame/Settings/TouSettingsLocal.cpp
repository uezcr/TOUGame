...
#include "Development/TouPlatformEmulationSettings.h"
#include "SoundControlBus.h"
#include "AudioModulationStatics.h"
#include "Audio/TouAudioSettings.h"
#include "Audio/TouAudioMixEffectsSubsystem.h"
#include "EnhancedActionKeyMapping.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(TouSettingsLocal)
...
#if WITH_EDITOR
static TAutoConsoleVariable<bool> CVarApplyFrameRateSettingsInPIE(TEXT("Tou.Settings.ApplyFrameRateSettingsInPIE"),
	false,
	TEXT("Should we apply frame rate settings in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyFrontEndPerformanceOptionsInPIE(TEXT("Tou.Settings.ApplyFrontEndPerformanceOptionsInPIE"),
	false,
	TEXT("Do we apply front-end specific performance options in PIE?"),
	ECVF_Default);

static TAutoConsoleVariable<bool> CVarApplyDeviceProfilesInPIE(TEXT("Tou.Settings.ApplyDeviceProfilesInPIE"),
	false,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenTargetFps(
	TEXT("Tou.DeviceProfile.Console.TargetFPS"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenFrameSyncType(
	TEXT("Tou.DeviceProfile.Console.FrameSyncType"),
	-1,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileDefaultFrameRate(
	TEXT("Tou.DeviceProfile.Mobile.DefaultFrameRate"),
	30,
...
static TAutoConsoleVariable<int32> CVarDeviceProfileDrivenMobileMaxFrameRate(
	TEXT("Tou.DeviceProfile.Mobile.MaxFrameRate"),
	30,
...
static TAutoConsoleVariable<FString> CVarMobileQualityLimits(
	TEXT("Tou.DeviceProfile.Mobile.OverallQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityLimits(
	TEXT("Tou.DeviceProfile.Mobile.ResolutionQualityLimits"),
	TEXT(""),
...
static TAutoConsoleVariable<FString> CVarMobileResolutionQualityRecommendation(
	TEXT("Tou.DeviceProfile.Mobile.ResolutionQualityRecommendation"),
	TEXT("0:75"),
...
FTouScalabilitySnapshot::FTouScalabilitySnapshot()
{
...
namespace TouSettingsHelpers
{
	...
	void FillScalabilitySettingsFromDeviceProfile(FTouScalabilitySnapshot& Mode, const FString& Suffix = FString())
	{
		static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
	...
	int32 ConstrainFrameRateToBeCompatibleWithOverallQuality(int32 FrameRate, int32 OverallQuality)
	{
		...
		int32 LimitIndex = PossibleRates.FindLastByPredicate([=](const int32& TestRate)
		{
			...
			const bool bIsSupported = UTouSettingsLocal::IsSupportedMobileFramePace(TestRate);

			return bAtOrBelowDesiredRate && bQualityDoesntExceedLimit && bIsSupported;
		});

		return PossibleRates.IsValidIndex(LimitIndex) ? PossibleRates[LimitIndex] : UTouSettingsLocal::GetDefaultMobileFrameRate();
	}
...
UTouSettingsLocal::UTouSettingsLocal()
{
...
void UTouSettingsLocal::SetToDefaults()
{
	Super::SetToDefaults();
...
void UTouSettingsLocal::LoadSettings(bool bForceReload)
{
	Super::LoadSettings(bForceReload);

	// Console platforms use rhi.SyncInterval to limit framerate
	const UTouPlatformSpecificRenderingSettings* PlatformSettings = UTouPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ETouFramePacingMode::ConsoleStyle)
	{
		FrameRateLimit = 0.0f;
	}
	...
	TouSettingsHelpers::FillScalabilitySettingsFromDeviceProfile(DeviceDefaultScalabilitySettings);
...
void UTouSettingsLocal::ResetToCurrentSettings()
{
...
void UTouSettingsLocal::BeginDestroy()
{
...
UTouSettingsLocal* UTouSettingsLocal::Get()
{
	return GEngine ? CastChecked<UTouSettingsLocal>(GEngine->GetGameUserSettings()) : nullptr;
}

void UTouSettingsLocal::ConfirmVideoMode()
{
...
float UTouSettingsLocal::GetEffectiveFrameRateLimit()
{
	const UTouPlatformSpecificRenderingSettings* PlatformSettings = UTouPlatformSpecificRenderingSettings::Get();
	...
	if (PlatformSettings->FramePacingMode == ETouFramePacingMode::DesktopStyle)
	{
...
int32 UTouSettingsLocal::GetHighestLevelOfAnyScalabilityChannel() const
{
	return TouSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(ScalabilityQuality);
}

void UTouSettingsLocal::OverrideQualityLevelsToScalabilityMode(const FTouScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UTouSettingsLocal::ClampQualityLevelsToDeviceProfile(const Scalability::FQualityLevels& ClampLevels, Scalability::FQualityLevels& InOutLevels)
{
	static_assert(sizeof(Scalability::FQualityLevels) == 88, "This function may need to be updated to account for new members");
...
void UTouSettingsLocal::OnExperienceLoaded()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UTouSettingsLocal::OnHotfixDeviceProfileApplied()
{
	ReapplyThingsDueToPossibleDeviceProfileChange();
}

void UTouSettingsLocal::ReapplyThingsDueToPossibleDeviceProfileChange()
{
	ApplyNonResolutionSettings();
}

void UTouSettingsLocal::SetShouldUseFrontendPerformanceSettings(bool bInFrontEnd)
{
	bInFrontEndForPerformancePurposes = bInFrontEnd;
	UpdateEffectiveFrameRateLimit();
}

bool UTouSettingsLocal::ShouldUseFrontendPerformanceSettings() const
{
#if WITH_EDITOR
...
ETouStatDisplayMode UTouSettingsLocal::GetPerfStatDisplayState(ETouDisplayablePerformanceStat Stat) const
{
	if (const ETouStatDisplayMode* pMode = DisplayStatList.Find(Stat))
	{
		return *pMode;
	}
	else
	{
		return ETouStatDisplayMode::Hidden;
	}
}

void UTouSettingsLocal::SetPerfStatDisplayState(ETouDisplayablePerformanceStat Stat, ETouStatDisplayMode DisplayMode)
{
	if (DisplayMode == ETouStatDisplayMode::Hidden)
	{
...
float UTouSettingsLocal::GetDisplayGamma() const
{
	return DisplayGamma;
}

void UTouSettingsLocal::SetDisplayGamma(float InGamma)
{
	DisplayGamma = InGamma;
	ApplyDisplayGamma();
}

void UTouSettingsLocal::ApplyDisplayGamma()
{
	if (GEngine)
	{
...
float UTouSettingsLocal::GetFrameRateLimit_OnBattery() const
{
	return FrameRateLimit_OnBattery;
}

void UTouSettingsLocal::SetFrameRateLimit_OnBattery(float NewLimitFPS)
{
	FrameRateLimit_OnBattery = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UTouSettingsLocal::GetFrameRateLimit_InMenu() const
{
	return FrameRateLimit_InMenu;
}

void UTouSettingsLocal::SetFrameRateLimit_InMenu(float NewLimitFPS)
{
	FrameRateLimit_InMenu = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UTouSettingsLocal::GetFrameRateLimit_WhenBackgrounded() const
{
	return FrameRateLimit_WhenBackgrounded;
}

void UTouSettingsLocal::SetFrameRateLimit_WhenBackgrounded(float NewLimitFPS)
{
	FrameRateLimit_WhenBackgrounded = NewLimitFPS;
	UpdateEffectiveFrameRateLimit();
}

float UTouSettingsLocal::GetFrameRateLimit_Always() const
{
	return GetFrameRateLimit();
}

void UTouSettingsLocal::SetFrameRateLimit_Always(float NewLimitFPS)
{
	SetFrameRateLimit(NewLimitFPS);
	UpdateEffectiveFrameRateLimit();
}

void UTouSettingsLocal::UpdateEffectiveFrameRateLimit()
{
...
int32 UTouSettingsLocal::GetDefaultMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileDefaultFrameRate.GetValueOnGameThread();
}

int32 UTouSettingsLocal::GetMaxMobileFrameRate()
{
	return CVarDeviceProfileDrivenMobileMaxFrameRate.GetValueOnGameThread();
}

bool UTouSettingsLocal::IsSupportedMobileFramePace(int32 TestFPS)
{
...
int32 UTouSettingsLocal::GetFirstFrameRateWithQualityLimit() const
{
	return TouSettingsHelpers::GetFirstFrameRateWithQualityLimit();
}

int32 UTouSettingsLocal::GetLowestQualityWithFrameRateLimit() const
{
	return TouSettingsHelpers::GetLowestQualityWithFrameRateLimit();
}

void UTouSettingsLocal::ResetToMobileDeviceDefaults()
{
	// Reset frame rate
...
int32 UTouSettingsLocal::GetMaxSupportedOverallQualityLevel() const
{
	const UTouPlatformSpecificRenderingSettings* PlatformSettings = UTouPlatformSpecificRenderingSettings::Get();
	if ((PlatformSettings->FramePacingMode == ETouFramePacingMode::MobileStyle) && DeviceDefaultScalabilitySettings.bHasOverrides)
	{
		return TouSettingsHelpers::GetHighestLevelOfAnyScalabilityChannel(DeviceDefaultScalabilitySettings.Qualities);
	}
	else
	{
		return 3;
...
void UTouSettingsLocal::SetMobileFPSMode(int32 NewLimitFPS)
{
	const UTouPlatformSpecificRenderingSettings* PlatformSettings = UTouPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ETouFramePacingMode::MobileStyle)
	{
...
void UTouSettingsLocal::SetDesiredMobileFrameRateLimit(int32 NewLimitFPS)
{
...
void UTouSettingsLocal::ClampMobileFPSQualityLevels(bool bWriteBack)
{
	const UTouPlatformSpecificRenderingSettings* PlatformSettings = UTouPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ETouFramePacingMode::MobileStyle)
	{
		const int32 QualityLimit = TouSettingsHelpers::GetApplicableOverallQualityLimit(DesiredMobileFrameRateLimit);
...
void UTouSettingsLocal::ClampMobileQuality()
{
	const UTouPlatformSpecificRenderingSettings* PlatformSettings = UTouPlatformSpecificRenderingSettings::Get();
	if (PlatformSettings->FramePacingMode == ETouFramePacingMode::MobileStyle)
	{
...
void UTouSettingsLocal::ClampMobileResolutionQuality(int32 TargetFPS)
{
	// Clamp mobile resolution quality
	float MaxMobileResQuality = TouSettingsHelpers::GetApplicableResolutionQualityLimit(TargetFPS);
...
void UTouSettingsLocal::RemapMobileResolutionQuality(int32 FromFPS, int32 ToFPS)
{
	...
	float MaxScaleValue = 0.0f;
	GetResolutionScaleInformationEx(CurrentScaleNormalized, CurrentScaleValue, MinScaleValue, MaxScaleValue);
	float FromMaxMobileResQuality = TouSettingsHelpers::GetApplicableResolutionQualityLimit(FromFPS);
	float ToMaxMobileResQuality = TouSettingsHelpers::GetApplicableResolutionQualityLimit(ToFPS);
...
FString UTouSettingsLocal::GetDesiredDeviceProfileQualitySuffix() const
{
	return DesiredUserChosenDeviceProfileSuffix;
}

void UTouSettingsLocal::SetDesiredDeviceProfileQualitySuffix(const FString& InDesiredSuffix)
{
	DesiredUserChosenDeviceProfileSuffix = InDesiredSuffix;
}

void UTouSettingsLocal::SetHeadphoneModeEnabled(bool bEnabled)
{
...
bool UTouSettingsLocal::IsHeadphoneModeEnabled() const
{
	return bUseHeadphoneMode;
}

bool UTouSettingsLocal::CanModifyHeadphoneModeEnabled() const
{
	static IConsoleVariable* BinauralSpatializationDisabledCVar = IConsoleManager::Get().FindConsoleVariable(TEXT("au.DisableBinauralSpatialization"));
	...
	const bool bBinauralSettingControlledByOS = TouSettingsHelpers::HasPlatformTrait(TAG_Platform_Trait_BinauralSettingControlledByOS);

	return bHRTFOptionAvailable && !bBinauralSettingControlledByOS;
}

bool UTouSettingsLocal::IsHDRAudioModeEnabled() const
{
	return bUseHDRAudioMode;
}

void UTouSettingsLocal::SetHDRAudioModeEnabled(bool bEnabled)
{
	...
	if (GEngine)
	{
		...
		{
			if (UTouAudioMixEffectsSubsystem* TouAudioMixEffectsSubsystem = World->GetSubsystem<UTouAudioMixEffectsSubsystem>())
			{
				TouAudioMixEffectsSubsystem->ApplyDynamicRangeEffectsChains(bEnabled);
			}
		}
	}
}

bool UTouSettingsLocal::CanRunAutoBenchmark() const
{
	const UTouPlatformSpecificRenderingSettings* PlatformSettings = UTouPlatformSpecificRenderingSettings::Get();
	return PlatformSettings->bSupportsAutomaticVideoQualityBenchmark;
}

bool UTouSettingsLocal::ShouldRunAutoBenchmarkAtStartup() const
{
...
void UTouSettingsLocal::RunAutoBenchmark(bool bSaveImmediately)
{
...
void UTouSettingsLocal::ApplyScalabilitySettings()
{
	Scalability::SetQualityLevels(ScalabilityQuality);
}

float UTouSettingsLocal::GetOverallVolume() const
{
	return OverallVolume;
}

void UTouSettingsLocal::SetOverallVolume(float InVolume)
{
...
float UTouSettingsLocal::GetMusicVolume() const
{
	return MusicVolume;
}

void UTouSettingsLocal::SetMusicVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UTouSettingsLocal::GetSoundFXVolume() const
{
	return SoundFXVolume;
}

void UTouSettingsLocal::SetSoundFXVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UTouSettingsLocal::GetDialogueVolume() const
{
	return DialogueVolume;
}

void UTouSettingsLocal::SetDialogueVolume(float InVolume)
{
	// Cache the incoming volume value
...
float UTouSettingsLocal::GetVoiceChatVolume() const
{
	return VoiceChatVolume;
}

void UTouSettingsLocal::SetVoiceChatVolume(float InVolume)
{
	// Cache the incoming volume value
...
void UTouSettingsLocal::SetVolumeForControlBus(USoundControlBus* InSoundControlBus, float InVolume)
{
	// Check to see if references to the control buses and control bus mixes have been loaded yet
...
void UTouSettingsLocal::SetAudioOutputDeviceId(const FString& InAudioOutputDeviceId)
{
	AudioOutputDeviceId = InAudioOutputDeviceId;
	OnAudioOutputDeviceChanged.Broadcast(InAudioOutputDeviceId);
}

void UTouSettingsLocal::ApplySafeZoneScale()
{
	SSafeZone::SetGlobalSafeZoneScale(GetSafeZone());
}

void UTouSettingsLocal::ApplyNonResolutionSettings()
{
...
int32 UTouSettingsLocal::GetOverallScalabilityLevel() const
{
...
void UTouSettingsLocal::SetOverallScalabilityLevel(int32 Value)
{
	...
	if (PlatformSettings->FramePacingMode == ETouFramePacingMode::MobileStyle)
	{
		...
		const int32 ConstrainedFrameRateLimit = TouSettingsHelpers::ConstrainFrameRateToBeCompatibleWithOverallQuality(DesiredMobileFrameRateLimit, Value);
...
void UTouSettingsLocal::SetControllerPlatform(const FName InControllerPlatform)
{
...
FName UTouSettingsLocal::GetControllerPlatform() const
{
	return ControllerPlatform;
}

void UTouSettingsLocal::LoadUserControlBusMix()
{
	if (GEngine)
	{
		if (const UWorld* World = GEngine->GetCurrentPlayWorld())
		{
			if (const UTouAudioSettings* TouAudioSettings = GetDefault<UTouAudioSettings>())
			{
				...
				if (UObject* ObjPath = TouAudioSettings->OverallVolumeControlBus.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Overall Control Bus reference missing from Tou Audio Settings."));
					}
				}

				if (UObject* ObjPath = TouAudioSettings->MusicVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Music Control Bus reference missing from Tou Audio Settings."));
					}
				}

				if (UObject* ObjPath = TouAudioSettings->SoundFXVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("SoundFX Control Bus reference missing from Tou Audio Settings."));
					}
				}

				if (UObject* ObjPath = TouAudioSettings->DialogueVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("Dialogue Control Bus reference missing from Tou Audio Settings."));
					}
				}

				if (UObject* ObjPath = TouAudioSettings->VoiceChatVolumeControlBus.TryLoad())
				{
					if (USoundControlBus* SoundControlBus = Cast<USoundControlBus>(ObjPath))
					{
					...
					{
						ensureMsgf(SoundControlBus, TEXT("VoiceChat Control Bus reference missing from Tou Audio Settings."));
					}
				}

				if (UObject* ObjPath = TouAudioSettings->UserSettingsControlBusMix.TryLoad())
				{
					...
					{
						ensureMsgf(SoundControlBusMix, TEXT("User Settings Control Bus Mix reference missing from Tou Audio Settings."));
					}
				}
			}
		}
...
void UTouSettingsLocal::OnAppActivationStateChanged(bool bIsActive)
{
	// We might want to adjust the frame rate when the app loses/gains focus on multi-window platforms
	UpdateEffectiveFrameRateLimit();
}

void UTouSettingsLocal::UpdateGameModeDeviceProfileAndFps()
{
#if WITH_EDITOR
	...
	const UTouPlatformSpecificRenderingSettings* PlatformSettings = UTouPlatformSpecificRenderingSettings::Get();
	const TArray<FTouQualityDeviceProfileVariant>& UserFacingVariants = PlatformSettings->UserFacingDeviceProfileOptions;
	...
	int32 SuffixIndex = UserFacingVariants.IndexOfByPredicate([&](const FTouQualityDeviceProfileVariant& Data){ return Data.DeviceProfileSuffix == UserChosenDeviceProfileSuffix; });
	...
	if (GIsEditor)
	{
		const UTouPlatformEmulationSettings* Settings = GetDefault<UTouPlatformEmulationSettings>();
	...
	{
	case ETouFramePacingMode::MobileStyle:
		UpdateMobileFramePacing();
		break;
	case ETouFramePacingMode::ConsoleStyle:
		UpdateConsoleFramePacing();
		break;
	case ETouFramePacingMode::DesktopStyle:
		UpdateDesktopFramePacing();
		break;
	}
}

void UTouSettingsLocal::UpdateConsoleFramePacing()
{
...
void UTouSettingsLocal::UpdateDesktopFramePacing()
{
...
void UTouSettingsLocal::UpdateMobileFramePacing()
{
...
void UTouSettingsLocal::UpdateDynamicResFrameTime(float TargetFPS)
{
