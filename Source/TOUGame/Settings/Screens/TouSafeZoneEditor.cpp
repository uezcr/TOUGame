...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TouSafeZoneEditor)
...
UTouSafeZoneEditor::UTouSafeZoneEditor(const FObjectInitializer& Initializer)
	: Super(Initializer)
{
...
void UTouSafeZoneEditor::NativeOnInitialized()
{
	Super::NativeOnInitialized();
	Switcher_SafeZoneMessage->SetActiveWidget(RichText_Default);
}

void UTouSafeZoneEditor::NativeOnActivated()
{
	Super::NativeOnActivated();

	SSafeZone::SetGlobalSafeZoneScale(UTouSettingsLocal::Get()->GetSafeZone());
	
	Button_Done->OnClicked().AddUObject(this, &UTouSafeZoneEditor::HandleDoneClicked);

	Button_Back->SetVisibility((bCanCancel)? ESlateVisibility::Visible : ESlateVisibility::Collapsed);
	if (bCanCancel)
	{
		Button_Back->OnClicked().AddUObject(this, &UTouSafeZoneEditor::HandleBackClicked);
	}
}

bool UTouSafeZoneEditor::ExecuteActionForSetting_Implementation(FGameplayTag ActionTag, UGameSetting* InSetting)
{
...
FReply UTouSafeZoneEditor::NativeOnAnalogValueChanged(const FGeometry& InGeometry, const FAnalogInputEvent& InAnalogEvent)
{
...
FReply UTouSafeZoneEditor::NativeOnMouseWheel(const FGeometry& InGeometry, const FPointerEvent& InMouseEvent)
{
...
void UTouSafeZoneEditor::HandleInputModeChanged(ECommonInputType InInputType)
{
...
void UTouSafeZoneEditor::HandleBackClicked()
{
	DeactivateWidget();
	SSafeZone::SetGlobalSafeZoneScale(UTouSettingsLocal::Get()->GetSafeZone());
}

void UTouSafeZoneEditor::HandleDoneClicked()
{
	...
	{
		UTouSettingsLocal::Get()->SetSafeZone(SSafeZone::GetGlobalSafeZoneScale().Get(1.0f));
	}
