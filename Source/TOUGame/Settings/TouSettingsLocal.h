...
#include "TouSettingsLocal.generated.h"
...
USTRUCT()
struct FTouScalabilitySnapshot
{
	GENERATED_BODY()

	FTouScalabilitySnapshot();
 ...
 */
UCLASS()
class UTouSettingsLocal : public UGameUserSettings
{
	GENERATED_BODY()
...
public:
	...
	ETouStatDisplayMode GetPerfStatDisplayState(ETouDisplayablePerformanceStat Stat) const;
	
	/** Sets the display mode for the specified performance stat */
	void SetPerfStatDisplayState(ETouDisplayablePerformanceStat Stat, ETouStatDisplayMode DisplayMode);

	/** Fired when the display state for a performance stat has changed, or the settings are applied */
	DECLARE_EVENT(UTouSettingsLocal, FPerfStatSettingsChanged);
...
private:
	...
	UPROPERTY(Config)
	TMap<ETouDisplayablePerformanceStat, ETouStatDisplayMode> DisplayStatList;
...
private:
	...
	void OverrideQualityLevelsToScalabilityMode(const FTouScalabilitySnapshot& InMode, Scalability::FQualityLevels& InOutLevels);
...
private:
	...
	FTouScalabilitySnapshot DeviceDefaultScalabilitySettings;
...
private:
	...
	// Audio - Volume
public:
	DECLARE_EVENT_OneParam(UTouSettingsLocal, FAudioDeviceChanged, const FString& /*DeviceId*/);
