...
#include "AbilitySystem/TouGameplayCueManager.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TouGameplayCueManager)

//////////////////////////////////////////////////////////////////////

enum class ETouEditorLoadMode
{
...
namespace TouGameplayCueManagerCvars
{
	static FAutoConsoleCommand CVarDumpGameplayCues(
		TEXT("Tou.DumpGameplayCues"),
		TEXT("Shows all assets that were loaded via TouGameplayCueManager and are currently in memory."),
		FConsoleCommandWithArgsDelegate::CreateStatic(UTouGameplayCueManager::DumpGameplayCues));

	static ETouEditorLoadMode LoadMode = ETouEditorLoadMode::LoadUpfront;
}
...
UTouGameplayCueManager::UTouGameplayCueManager(const FObjectInitializer& ObjectInitializer)
: Super(ObjectInitializer)
{
}

UTouGameplayCueManager* UTouGameplayCueManager::Get()
{
	return Cast<UTouGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
}

void UTouGameplayCueManager::OnCreated()
{
	Super::OnCreated();
...
void UTouGameplayCueManager::LoadAlwaysLoadedCues()
{
	if (ShouldDelayLoadGameplayCues())
	{
		...
		for (const FName& CueTagName : AdditionalAlwaysLoadedCueTags)
		{
			...
			{
				UE_LOG(LogTou, Warning, TEXT("UTouGameplayCueManager::AdditionalAlwaysLoadedCueTags contains invalid tag %s"), *CueTagName.ToString());
			}
		}
	}
}

bool UTouGameplayCueManager::ShouldAsyncLoadRuntimeObjectLibraries() const
{
	switch (TouGameplayCueManagerCvars::LoadMode)
	{
	case ETouEditorLoadMode::LoadUpfront:
		return true;
	case ETouEditorLoadMode::PreloadAsCuesAreReferenced_GameOnly:
#if WITH_EDITOR
		if (GIsEditor)
		{
			return false;
...
#endif
		break;
	case ETouEditorLoadMode::PreloadAsCuesAreReferenced:
		break;
	}
...
bool UTouGameplayCueManager::ShouldSyncLoadMissingGameplayCues() const
{
	return false;
}

bool UTouGameplayCueManager::ShouldAsyncLoadMissingGameplayCues() const
{
	return true;
}

void UTouGameplayCueManager::DumpGameplayCues(const TArray<FString>& Args)
{
		UTouGameplayCueManager* GCM = Cast<UTouGameplayCueManager>(UAbilitySystemGlobals::Get().GetGameplayCueManager());
	if (!GCM)
	{
		UE_LOG(LogTou, Error, TEXT("DumpGameplayCues failed. No UTouGameplayCueManager found."));
		return;
	}

	const bool bIncludeRefs = Args.Contains(TEXT("Refs"));

	UE_LOG(LogTou, Log, TEXT("=========== Dumping Always Loaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->AlwaysLoadedCues)
	{
		UE_LOG(LogTou, Log, TEXT("  %s"), *GetPathNameSafe(CueClass));
	}

	UE_LOG(LogTou, Log, TEXT("=========== Dumping Preloaded Gameplay Cue Notifies ==========="));
	for (UClass* CueClass : GCM->PreloadedCues)
	{
		...
		UE_LOG(LogTou, Log, TEXT("  %s (%d refs)"), *GetPathNameSafe(CueClass), NumRefs);
		if (bIncludeRefs && ReferencerSet)
		{
			for (const FObjectKey& Ref : *ReferencerSet)
			{
				UObject* RefObject = Ref.ResolveObjectPtr();
				UE_LOG(LogTou, Log, TEXT("    ^- %s"), *GetPathNameSafe(RefObject));
			}
		}
	}

	UE_LOG(LogTou, Log, TEXT("=========== Dumping Gameplay Cue Notifies loaded on demand ==========="));
	...
	if (GCM->RuntimeGameplayCueObjectLibrary.CueSet)
	{
		for (const FGameplayCueNotifyData& CueData : GCM->RuntimeGameplayCueObjectLibrary.CueSet->GameplayCueData)
		{
			if (CueData.LoadedGameplayCueClass && !GCM->AlwaysLoadedCues.Contains(CueData.LoadedGameplayCueClass) && !GCM->PreloadedCues.Contains(CueData.LoadedGameplayCueClass))
			{
				...
				UE_LOG(LogTou, Log, TEXT("  %s"), *CueData.LoadedGameplayCueClass->GetPathName());
			}
		}
	}

	UE_LOG(LogTou, Log, TEXT("=========== Gameplay Cue Notify summary ==========="));
	UE_LOG(LogTou, Log, TEXT("  ... %d cues in always loaded list"), GCM->AlwaysLoadedCues.Num());
	UE_LOG(LogTou, Log, TEXT("  ... %d cues in preloaded list"), GCM->PreloadedCues.Num());
	UE_LOG(LogTou, Log, TEXT("  ... %d cues loaded on demand"), NumMissingCuesLoaded);
	UE_LOG(LogTou, Log, TEXT("  ... %d cues in total"), GCM->AlwaysLoadedCues.Num() + GCM->PreloadedCues.Num() + NumMissingCuesLoaded);
}

void UTouGameplayCueManager::OnGameplayTagLoaded(const FGameplayTag& Tag)
{
		...
		TGraphTask<FGameplayCueTagThreadSynchronizeGraphTask>::CreateTask().ConstructAndDispatchWhenReady([]()
			{
				if (GIsRunning)
				{
					if (UTouGameplayCueManager* StrongThis = Get())
					{
...
void UTouGameplayCueManager::HandlePostGarbageCollect()
{
...
void UTouGameplayCueManager::ProcessLoadedTags()
{
	...
	if (GIsRunning)
	{
		...
		{
			UE_LOG(LogTou, Warning, TEXT("UTouGameplayCueManager::OnGameplayTagLoaded processed loaded tag(s) but RuntimeGameplayCueObjectLibrary.CueSet was null. Skipping processing."));
		}
	}
}

void UTouGameplayCueManager::ProcessTagToPreload(const FGameplayTag& Tag, UObject* OwningObject)
{
	switch (TouGameplayCueManagerCvars::LoadMode)
	{
	case ETouEditorLoadMode::LoadUpfront:
...
void UTouGameplayCueManager::OnPreloadCueComplete(FSoftObjectPath Path, TWeakObjectPtr<UObject> OwningObject,
...
void UTouGameplayCueManager::RegisterPreloadedCue(UClass* LoadedGameplayCueClass, UObject* OwningObject)
{
...
void UTouGameplayCueManager::HandlePostLoadMap(UWorld* NewWorld)
{
	if (RuntimeGameplayCueObjectLibrary.CueSet)
	{
...
void UTouGameplayCueManager::UpdateDelayLoadDelegateListeners()
{
...
bool UTouGameplayCueManager::ShouldDelayLoadGameplayCues() const
{
...
void UTouGameplayCueManager::RefreshGameplayCuePrimaryAsset()
{
