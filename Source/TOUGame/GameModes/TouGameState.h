...
#include "TouGameState.generated.h"
...
class UTouExperienceManagerComponent;
class UObject;
struct FFrame;
 ...
 */
UCLASS(Config = Game)
class TOUGAME_API ATouGameState : public AModularGameStateBase, public IAbilitySystemInterface
{
	GENERATED_BODY()

public:

	ATouGameState(const FObjectInitializer& ObjectInitializer = FObjectInitializer::Get());
	...
	UFUNCTION(BlueprintCallable, Category = "Tou|GameState")
	UTouAbilitySystemComponent* GetTouAbilitySystemComponent() const { return AbilitySystemComponent; }

	// Send a message that all clients will (probably) get
	// (use only for client notifications like eliminations, server join messages, etc... that can handle being lost)
	UFUNCTION(NetMulticast, Unreliable, BlueprintCallable, Category = "Tou|GameState")
	void MulticastMessageToClients(const FTouVerbMessage Message);

	// Send a message that all clients will be guaranteed to get
	// (use only for client notifications that cannot handle being lost)
	UFUNCTION(NetMulticast, Reliable, BlueprintCallable, Category = "Tou|GameState")
	void MulticastReliableMessageToClients(const FTouVerbMessage Message);
...
private:
	...
	UPROPERTY()
	TObjectPtr<UTouExperienceManagerComponent> ExperienceManagerComponent;

	// The ability system component subobject for game-wide things (primarily gameplay cues)
	UPROPERTY(VisibleAnywhere, Category = "Tou|GameState")
