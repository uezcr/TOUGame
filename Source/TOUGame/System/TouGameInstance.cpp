...
#include "System/TouGameInstance.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TouGameInstance)

namespace Tou
{
	static bool bTestEncryption = false;
	static FAutoConsoleVariableRef CVarTouTestEncryption(
		TEXT("Tou.TestEncryption"),
		bTestEncryption,
	...
	static FAutoConsoleVariableRef CVarTouUseDTLSEncryption(
		TEXT("Tou.UseDTLSEncryption"),
		bUseDTLSEncryption,
		TEXT("Set to true if using Tou.TestEncryption and the DTLS packet handler."),
		ECVF_Default);
	...
	static FAutoConsoleVariableRef CVarTouTestDTLSFingerprint(
		TEXT("Tou.TestDTLSFingerprint"),
...
UTouGameInstance::UTouGameInstance(const FObjectInitializer& ObjectInitializer)
: Super(ObjectInitializer)
{
}

ATouPlayerController* UTouGameInstance::GetPrimaryPlayerController() const
{
	return Cast<ATouPlayerController>(Super::GetPrimaryPlayerController(false));
}

bool UTouGameInstance::CanJoinRequestedSession() const
{
...
void UTouGameInstance::HandlerUserInitialized(const UCommonUserInfo* UserInfo, bool bSuccess, FText Error,
	ECommonUserPrivilege RequestedPrivilege, ECommonUserOnlineContext OnlineContext)
{
	...
	{
		UTouLocalPlayer* LocalPlayer = Cast<UTouLocalPlayer>(GetLocalPlayerByIndex(UserInfo->LocalPlayerIndex));
...
void UTouGameInstance::ReceivedNetworkEncryptionToken(const FString& EncryptionToken,
	const FOnEncryptionKeyResponse& Delegate)
{
	...
	{
#if UE_WITH_DTLS
		if (Tou::bUseDTLSEncryption)
		{
			TSharedPtr<FDTLSCertificate> Cert;

			if (Tou::bTestDTLSFingerprint)
			{
			...
			else
			{
				...
				const FString CertPath = FPaths::ProjectContentDir() / TEXT("DTLS") / TEXT("TouTest.pem");
...
void UTouGameInstance::ReceivedNetworkEncryptionAck(const FOnEncryptionKeyResponse& Delegate)
{
	// This is a simple implementation to demonstrate using encryption for game traffic using a hardcoded key.
	// For a complete implementation, you would likely want to retrieve the encryption key from a secure source,
	// such as from a web service over HTTPS. This could be done in this function, even asynchronously - just
...
void UTouGameInstance::Init()
{
	...
	{
		ComponentManager->RegisterInitState(TouGameplayTags::InitState_Spawned, false, FGameplayTag());
		ComponentManager->RegisterInitState(TouGameplayTags::InitState_DataAvailable, false, TouGameplayTags::InitState_Spawned);
		ComponentManager->RegisterInitState(TouGameplayTags::InitState_DataInitialized, false, TouGameplayTags::InitState_DataAvailable);
		ComponentManager->RegisterInitState(TouGameplayTags::InitState_GameplayReady, false, TouGameplayTags::InitState_DataInitialized);
	}
	...
	{
		SessionSubsystem->OnPreClientTravelEvent.AddUObject(this, &UTouGameInstance::OnPreClientTravelToSession);
	}
}

void UTouGameInstance::Shutdown()
{
	if (UCommonSessionSubsystem* SessionSubsystem = GetSubsystem<UCommonSessionSubsystem>())
	{
...
void UTouGameInstance::OnPreClientTravelToSession(FString& URL)
{
	// Add debug encryption token if desired.
	if (Tou::bTestEncryption)
	{
#if UE_WITH_DTLS
		if (Tou::bUseDTLSEncryption)
		{
