...
DECLARE_DELEGATE_OneParam(FTouAssetManagerStartupJobSubstepProgress, float /*NewProgress*/);

struct FTouAssetManagerStartupJob
{
	FTouAssetManagerStartupJobSubstepProgress SubstepProgressDelegate;
	TFunction<void(const FTouAssetManagerStartupJob&, TSharedPtr<FStreamableHandle>&)> JobFunc;
	FString JobName;
	float JobWeight;
	mutable double LastUpdate = 0;

	/** Simple job that is all synchronous */
	FTouAssetManagerStartupJob(const FString& InJobName, const TFunction<void(const FTouAssetManagerStartupJob&, TSharedPtr<FStreamableHandle>&)>& InJobFunc, float InJobWeight)
		: JobFunc(InJobFunc)
		, JobName(InJobName)
