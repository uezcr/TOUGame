...
#include "TouInputModifiers.generated.h"

struct FInputActionValue;

class FProperty;
class UEnhancedPlayerInput;
class UTouAimSensitivityData;
class UObject;
...
class UTouSettingBasedScalar : public UInputModifier
{
	GENERATED_BODY()
...
/**
 * This is a deadzone input modifier that will have it's thresholds driven by what is in the Tou Shared game settings. 
 */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Tou Settings Driven Dead Zone"))
class UTouInputModifierDeadZone : public UInputModifier
{
	GENERATED_BODY()
...
UENUM()
enum class ETouTargetingType : uint8
{
...
/** Applies a scalar modifier based on the current gamepad settings in Tou Shared game settings.  */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Tou Gamepad Sensitivity"))
class UTouInputModifierGamepadSensitivity : public UInputModifier
{
	GENERATED_BODY()
public:
	...
	UPROPERTY(EditInstanceOnly, BlueprintReadWrite, Category=Settings, Config)
	ETouTargetingType TargetingType = ETouTargetingType::Normal;

	/** Asset that gives us access to the float scalar value being used for sensitivty */
	UPROPERTY(EditAnywhere, BlueprintReadOnly, meta=(AssetBundles="Client,Server"))
	TObjectPtr<const UTouAimSensitivityData> SensitivityLevelTable;

protected:
	virtual FInputActionValue ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime) override;
};

/** Applies an inversion of axis values based on a setting in the Tou Shared game settings */
UCLASS(NotBlueprintable, MinimalAPI, meta = (DisplayName = "Tou Aim Inversion Setting"))
class UTouInputModifierAimInversion : public UInputModifier
{
	GENERATED_BODY()
