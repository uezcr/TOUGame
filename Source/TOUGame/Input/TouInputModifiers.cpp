...
#include "TouInputModifiers.h"

#include "EnhancedPlayerInput.h"
#include "GameFramework/PlayerController.h"
#include "Input/TouAimSensitivityData.h"
#include "Player/TouLocalPlayer.h"
#include "Settings/TouSettingsShared.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(TouInputModifiers)

DEFINE_LOG_CATEGORY_STATIC(LogTouInputModifiers, Log, All);

//////////////////////////////////////////////////////////////////////
// TouInputModifiersHelpers

namespace TouInputModifiersHelpers
{
	/** Returns the owning TouLocalPlayer of an Enhanced Player Input pointer */
	static UTouLocalPlayer* GetLocalPlayer(const UEnhancedPlayerInput* PlayerInput)
	{
		if (PlayerInput)
		{
			if (APlayerController* PC = Cast<APlayerController>(PlayerInput->GetOuter()))
			{
				return Cast<UTouLocalPlayer>(PC->GetLocalPlayer());
			}
		}
		return nullptr;
	}
...
//////////////////////////////////////////////////////////////////////
// UTouSettingBasedScalar

FInputActionValue UTouSettingBasedScalar::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	...
	{
		if (UTouLocalPlayer* LocalPlayer = TouInputModifiersHelpers::GetLocalPlayer(PlayerInput))
		{
			const UClass* SettingsClass = UTouSettingsShared::StaticClass();
			UTouSettingsShared* SharedSettings = LocalPlayer->GetSharedSettings();
...
//////////////////////////////////////////////////////////////////////
// UTouInputModifierDeadZone

FInputActionValue UTouInputModifierDeadZone::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	EInputActionValueType ValueType = CurrentValue.GetValueType();
	UTouLocalPlayer* LocalPlayer = TouInputModifiersHelpers::GetLocalPlayer(PlayerInput);
	if (ValueType == EInputActionValueType::Boolean || !LocalPlayer)
	{
		return CurrentValue;
	}
	
	UTouSettingsShared* Settings = LocalPlayer->GetSharedSettings();
	ensure(Settings);
...
FLinearColor UTouInputModifierDeadZone::GetVisualizationColor_Implementation(FInputActionValue SampleValue, FInputActionValue FinalValue) const
{
...
//////////////////////////////////////////////////////////////////////
// UTouInputModifierGamepadSensitivity

FInputActionValue UTouInputModifierGamepadSensitivity::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	...
	const ETouGamepadSensitivity Sensitivity = (TargetingType == ETouTargetingType::Normal) ? Settings->GetGamepadLookSensitivityPreset() : Settings->GetGamepadTargetingSensitivityPreset();
...
//////////////////////////////////////////////////////////////////////
// UTouInputModifierAimInversion

FInputActionValue UTouInputModifierAimInversion::ModifyRaw_Implementation(const UEnhancedPlayerInput* PlayerInput, FInputActionValue CurrentValue, float DeltaTime)
{
	UTouLocalPlayer* LocalPlayer = TouInputModifiersHelpers::GetLocalPlayer(PlayerInput);
	if (!LocalPlayer)
	{
