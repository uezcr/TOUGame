#include "TouReticleWidgetBase.h"

#include "Inventory/TouInventoryItemInstance.h"
#include "Weapons/TouRangedWeaponInstance.h"
#include "Weapons/TouWeaponInstance.h"

#include UE_INLINE_GENERATED_CPP_BY_NAME(TouReticleWidgetBase)

UTouReticleWidgetBase::UTouReticleWidgetBase(const FObjectInitializer& ObjectInitializer)
	: Super(ObjectInitializer)
{
}

void UTouReticleWidgetBase::InitializeFromWeapon(UTouWeaponInstance* InWeapon)
{
	WeaponInstance = InWeapon;
	InventoryInstance = nullptr;
	if (WeaponInstance)
	{
		InventoryInstance = Cast<UTouInventoryItemInstance>(WeaponInstance->GetInstigator());
	}
	OnWeaponInitialized();
}


float UTouReticleWidgetBase::ComputeSpreadAngle() const
{
	if (const UTouRangedWeaponInstance* RangedWeapon = Cast<const UTouRangedWeaponInstance>(WeaponInstance))
	{
...
bool UTouReticleWidgetBase::HasFirstShotAccuracy() const
{
	if (const UTouRangedWeaponInstance* RangedWeapon = Cast<const UTouRangedWeaponInstance>(WeaponInstance))
	{
...
float UTouReticleWidgetBase::ComputeMaxScreenspaceSpreadRadius() const
{
