...
#include "TouPerformanceSettings.generated.h"
...
USTRUCT()
struct FTouQualityDeviceProfileVariant
{
	GENERATED_BODY()
...
USTRUCT()
struct FTouPerformanceStatGroup
{
	GENERATED_BODY()
...
class UTouPlatformSpecificRenderingSettings : public UPlatformSettings
{
	GENERATED_BODY()

public:
	UTouPlatformSpecificRenderingSettings();

	// Helper method to get the performance settings object, directed via platform settings
	static const UTouPlatformSpecificRenderingSettings* Get();
...
public:
	...
	// Note that this will usually be set from platform-specific ini files, not via the UI
	UPROPERTY(EditAnywhere, Config, Category=DeviceProfiles)
	TArray<FTouQualityDeviceProfileVariant> UserFacingDeviceProfileOptions;
	...
	UPROPERTY(EditAnywhere, Config, Category=VideoSettings)
	ETouFramePacingMode FramePacingMode = ETouFramePacingMode::DesktopStyle;
 ...
 */
UCLASS(config=Game, defaultconfig, meta=(DisplayName="Tou Performance Settings"))
class UTouPerformanceSettings : public UDeveloperSettingsBackedByCVars
{
	GENERATED_BODY()

public:
	UTouPerformanceSettings();
...
public:
	...
	TArray<FTouPerformanceStatGroup> UserFacingPerformanceStats;
};
