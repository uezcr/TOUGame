...
#include "TouReplaySubsystem.generated.h"
...
class TOUGAME_API UTouReplayListEntry : public UObject
{
	GENERATED_BODY()
...
UCLASS(BlueprintType)
class TOUGAME_API UTouReplayList : public UObject
{
	GENERATED_BODY()

public:
	UPROPERTY(BlueprintReadWrite, Category=Replays)
	TArray<TObjectPtr<UTouReplayListEntry>> Results;
};

/** Subsystem to handle recording/loading replays */
UCLASS()
class TOUGAME_API UTouReplaySubsystem : public UGameInstanceSubsystem
{
	GENERATED_BODY()
...
public:
	...
	void PlayReplay(UTouReplayListEntry* Replay);
