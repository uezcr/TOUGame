...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TouReplaySubsystem)

UE_DEFINE_GAMEPLAY_TAG_STATIC(TAG_Platform_Trait_ReplaySupport, "Platform.Trait.ReplaySupport");

UTouReplaySubsystem::UTouReplaySubsystem()
{
}

bool UTouReplaySubsystem::DoesPlatformSupportReplays()
{
...
FGameplayTag UTouReplaySubsystem::GetPlatformSupportTraitTag()
{
	return TAG_Platform_Trait_ReplaySupport.GetTag();
}

void UTouReplaySubsystem::PlayReplay(UTouReplayListEntry* Replay)
{
	if (Replay != nullptr)
	{
...
void UTouReplaySubsystem::RecordClientReplay(APlayerController* PlayerController)
{
	if (ensure(DoesPlatformSupportReplays() && PlayerController))
	{
		FText FriendlyNameText = FText::Format(NSLOCTEXT("Tou", "TouReplayName_Format", "Client Replay {0}"), FText::AsDateTime(FDateTime::UtcNow(), EDateTimeStyle::Short, EDateTimeStyle::Short));
		GetGameInstance()->StartRecordingReplay(FString(), FriendlyNameText.ToString());

		if (UTouLocalPlayer* TouLocalPlayer = Cast<UTouLocalPlayer>(PlayerController->GetLocalPlayer()))
		{
			// Start a cleanup of existing saved streams
			int32 NumToKeep = TouLocalPlayer->GetLocalSettings()->GetNumberOfReplaysToKeep();
			CleanupLocalReplays(TouLocalPlayer, NumToKeep);
		}
	}
}

void UTouReplaySubsystem::CleanupLocalReplays(ULocalPlayer* LocalPlayer, int32 NumReplaysToKeep)
{
...
void UTouReplaySubsystem::OnEnumerateStreamsCompleteForDelete(const FEnumerateStreamsResult& Result)
{
	...
	if (StreamsToDelete.Num() > DeletingReplaysNumberToKeep)
	{
		...
		UE_LOG(LogTou, Log, TEXT("TouReplaySubsystem asked to delete replay %s"), *ReplayName);
...
void UTouReplaySubsystem::OnDeleteReplay(const FDeleteFinishedStreamResult& DeleteResult)
{
	if (!CurrentReplayStreamer.IsValid() || !IsValid(LocalPlayerDeletingReplays))
	{
		// Lost context, don't do anything
	...
	else
	{
		...
		UE_LOG(LogTou, Warning, TEXT("Failed to delete replay with error %d!"), (int32)DeleteResult.Result);
...
void UTouReplaySubsystem::SeekInActiveReplay(float TimeInSeconds)
{
	if (UDemoNetDriver* DemoDriver = GetDemoDriver())
	{
...
float UTouReplaySubsystem::GetReplayLengthInSeconds() const
{
	if (UDemoNetDriver* DemoDriver = GetDemoDriver())
	{
...
float UTouReplaySubsystem::GetReplayCurrentTime() const
{
	if (UDemoNetDriver* DemoDriver = GetDemoDriver())
	{
...
UDemoNetDriver* UTouReplaySubsystem::GetDemoDriver() const
{
