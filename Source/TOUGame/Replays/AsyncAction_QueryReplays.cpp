...
void UAsyncAction_QueryReplays::Activate()
{
	...
	ResultList = NewObject<UTouReplayList>();
...
void UAsyncAction_QueryReplays::OnEnumerateStreamsComplete(const FEnumerateStreamsResult& Result)
{
	...
	{
		UTouReplayListEntry* NewReplayEntry = NewObject<UTouReplayListEntry>(ResultList);
		NewReplayEntry->StreamInfo = StreamInfo;
		ResultList->Results.Add(NewReplayEntry);
	}

	// Sort demo names by date
	Algo::SortBy(ResultList->Results, [](const TObjectPtr<UTouReplayListEntry>& Data) { return Data->StreamInfo.Timestamp.GetTicks(); }, TGreater<>());
