...
#include "TouAudioSettings.generated.h"
...
USTRUCT()
struct TOUGAME_API FTouSubmixEffectChainMap
{
	GENERATED_BODY()
 ...
 * 
 */
UCLASS(config = Game, defaultconfig, meta = (DisplayName = "TouAudioSettings"))
class TOUGAME_API UTouAudioSettings : public UDeveloperSettings
{
	GENERATED_BODY()
...
public:
	...
	TArray<FTouSubmixEffectChainMap> HDRAudioSubmixEffectChain;
	
	/** Submix Processing Chains to achieve low dynamic range audio output */
	UPROPERTY(config, EditAnywhere, Category = EffectSettings)
	TArray<FTouSubmixEffectChainMap> LDRAudioSubmixEffectChain;
