...
#include UE_INLINE_GENERATED_CPP_BY_NAME(TouTeamDisplayAsset)

void UTouTeamDisplayAsset::ApplyToMaterial(UMaterialInstanceDynamic* Material)
{
	if (Material)
	{
...
void UTouTeamDisplayAsset::ApplyToMeshComponent(UMeshComponent* MeshComponent)
{
	if (MeshComponent)
	{
		for (const auto& KVP : ScalarParameters)
...
void UTouTeamDisplayAsset::ApplyToNiagaraComponent(UNiagaraComponent* NiagaraComponent)
{
	if (NiagaraComponent)
	{
		for (const auto& KVP : ScalarParameters)
...
void UTouTeamDisplayAsset::ApplyToActor(AActor* TargetActor, bool bIncludeChildActors)
{
...
#if WITH_EDITOR
void UTouTeamDisplayAsset::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);

	for (UTouTeamSubsystem* TeamSubsystem : TObjectRange<UTouTeamSubsystem>())
	{
