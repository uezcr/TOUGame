...
#include "Teams/TouTeamStatics.h"
...
UAsyncAction_ObserveTeamColors* UAsyncAction_ObserveTeamColors::ObserveTeamColors(UObject* TeamAgent)
{
	...
	if (TeamAgent != nullptr)
	{
		...
		Action->TeamInterfacePtr = TWeakInterfacePtr<ITouTeamAgentInterface>(TeamAgent);
...
void UAsyncAction_ObserveTeamColors::SetReadyToDestroy()
{
	...
	if (ITouTeamAgentInterface* TeamInterface = TeamInterfacePtr.Get())
	{
...
void UAsyncAction_ObserveTeamColors::Activate()
{
	...
	UTouTeamDisplayAsset* CurrentDisplayAsset = nullptr;
	...
	if (ITouTeamAgentInterface* TeamInterface = TeamInterfacePtr.Get())
	{
		if (UWorld* World = GEngine->GetWorldFromContextObject(TeamInterfaceObj.Get(), EGetWorldErrorMode::LogAndReturnNull))
		{
			...
			CurrentDisplayAsset = UTouTeamStatics::GetTeamDisplayAsset(World, CurrentTeamIndex);
...
void UAsyncAction_ObserveTeamColors::BroadcastChange(int32 NewTeam, const UTouTeamDisplayAsset* DisplayAsset)
{
	UWorld* World = GEngine->GetWorldFromContextObject(TeamInterfaceObj.Get(), EGetWorldErrorMode::LogAndReturnNull);
	UTouTeamSubsystem* TeamSubsystem = UWorld::GetSubsystem<UTouTeamSubsystem>(World);
...
{
	UTouTeamDisplayAsset* DisplayAsset = UTouTeamStatics::GetTeamDisplayAsset(TeamAgent, NewTeam);
	BroadcastChange(NewTeam, DisplayAsset);
}

void UAsyncAction_ObserveTeamColors::OnDisplayAssetChanged(const UTouTeamDisplayAsset* DisplayAsset)
{
