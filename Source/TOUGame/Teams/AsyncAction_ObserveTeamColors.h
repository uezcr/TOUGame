...
class ITouTeamAgentInterface;
class UTouTeamDisplayAsset;
struct FFrame;

DECLARE_DYNAMIC_MULTICAST_DELEGATE_ThreeParams(FTeamColorObservedAsyncDelegate, bool, bTeamSet, int32, TeamId, const UTouTeamDisplayAsset*, DisplayAsset);
...
public:
	...
	//  - For anything that can ever belong to a team (implements ITouTeamAgentInterface),
...
private:
	void BroadcastChange(int32 NewTeam, const UTouTeamDisplayAsset* DisplayAsset);

	UFUNCTION()
	void OnWatchedAgentChangedTeam(UObject* TeamAgent, int32 OldTeam, int32 NewTeam);

	UFUNCTION()
	void OnDisplayAssetChanged(const UTouTeamDisplayAsset* DisplayAsset);

	TWeakInterfacePtr<ITouTeamAgentInterface> TeamInterfacePtr;
